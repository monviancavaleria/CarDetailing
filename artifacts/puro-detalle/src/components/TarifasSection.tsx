...
function LevelIcon({ level }: { level: Level }) {
  if (level === 'full') {
    return (
      <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-[#2CA5DC] shadow-[0_2px_10px_rgba(150,220,246,0.6)]">
  ...
  if (level === 'basic') {
    return (
      <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-[#C9CED6]/60 border border-[#8C96A3]/60">
        <Check className="w-3.5 h-3.5 text-[#2CA5DC]" strokeWidth={3} />
  ...
  return (
    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full border border-[#C9CED6]/80">
      <X className="w-3.5 h-3.5 text-[#2CA5DC]" strokeWidth={2.5} />
...
function PkgHeader({ pkg, highlighted }: { pkg: Pkg; highlighted: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-[#2CA5DC]" aria-hidden>★ </span>{pkg.badge}
      </span>
      )}
      <span
        className={`font-serif uppercase tracking-wider text-sm md:text-base leading-tight ${
          isBlue ? 'text-[#1D80C3]' : 'text-[#4A5462]'
        }`}
      >
        {pkg.name}
      </span>
      ...
      <span className="inline-flex items-center gap-1 text-[10px] text-muted-foreground font-sans">
        <Clock className="w-3 h-3 text-[#2CA5DC]" />
        {pkg.duration}
      </span>
    </div>
  );
    ...
    <section id="tarifas" className="py-24 relative z-10 scroll-mt-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10">
          ...
          {isFiltered && onClear && (
            <button
              type="button"
              onClick={onClear}
              className="mt-6 inline-flex items-center gap-2 px-5 py-2 rounded-full border border-[#0077D6]/30 bg-white/40 text-[11px] tracking-widest uppercase font-sans font-medium text-[#1D80C3] hover:bg-[#0077D6]/10 transition-colors"
            >
              Ver los 4 paquetes
            </button>
          )}
                    ...
                    <tr>
                      <td colSpan={cols.length + 1} className="pt-8 pb-3">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-xs tracking-[0.25em] uppercase font-sans font-semibold text-[#1D80C3]">
                            {group.title}
                          </span>
                          <span className="h-px flex-1 bg-[#0077D6]/15" />
                        </div>
                ...
                <tr key={label}>
                  <th scope="row" className="text-left align-middle py-4 pr-2">
                    <span className="flex items-center gap-3">
                      <Icon className="w-8 h-8 text-[#2CA5DC]" strokeWidth={1.5} />
                      <span>
                    ...
                    >
                      <span
                        className={`text-2xl md:text-3xl font-light ${
                          pkg.kind === 'blue' ? 'text-[#1D80C3]' : 'text-[#4A5462]'
                        }`}
                      >
                        {prices[idx]}
                      </span>
        ...
        <div className="text-center mt-14">
          <p className="font-script italic text-xl md:text-2xl text-[#1D80C3] mb-5">
